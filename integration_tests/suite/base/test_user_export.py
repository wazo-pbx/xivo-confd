...
def test_given_user_with_no_authentication_when_exporting_then_no_username(user):
    response = confd_csv.users.export.get()
    assert_that(
        response.csv(), has_item(has_entries(uuid=user['uuid'], username="")),
    )
...
def test_given_user_has_sccp_line_when_exporting_then_csv_has_line_fields(
    ...
    with a.line_endpoint_sccp(line, sccp), a.user_line(user, line):
        ...
        assert_that(
            ...
            has_item(
                has_entries(
                    uuid=user['uuid'], line_protocol="sccp", context=line['context']
                )
            ),
        )
...
@fixtures.sip()
@fixtures.extension()
def test_given_user_has_extension_when_exporting_then_csv_has_extension_fields(
    user, line, sip, extension
):
    auth.users.new(uuid=user['uuid'])
...
def test_given_user_has_incall_when_exporting_then_csv_has_incall_fields(
    user, line, sip, incall, extension
):
    auth.users.new(uuid=user['uuid'])
    with a.line_endpoint_sip(line, sip), a.user_line(user, line), a.incall_extension(
        incall, extension
    ), a.incall_user(incall, user):
        response = confd_csv.users.export.get()
        assert_that(
            response.csv(),
            has_item(
...
def test_given_user_has_multiple_incalls_when_exporting_then_csv_has_incall_fields(
    user, line, sip, incall1, incall2, extension1, extension2
):
    auth.users.new(uuid=user['uuid'])
    with a.line_endpoint_sip(line, sip), a.user_line(user, line), a.incall_extension(
        incall1, extension1
    ), a.incall_extension(incall2, extension2), a.incall_user(
        incall1, user
    ), a.incall_user(
        incall2, user
    ):
        ...
        assert_that(
            ...
            has_item(
                has_entries(
                    ...
                    incall_context=";".join(
                        [extension1['context'], extension2['context']]
                    ),
                )
            ),
        )
...
@fixtures.call_permission()
def test_given_user_has_multiple_call_permissions_when_exporting_then_csv_has_call_permission_field(
    user, perm1, perm2
):
    auth.users.new(uuid=user['uuid'])
